...
class BaseEngine(object):
    def __init__(self, engine_path):
        ...
        self.std = None
        self.n_classes = 17
        self.class_names = [ 'bj_bpmh', 'bj_bpps', 'bj_wkps', 'bjdsyc','jyz_pl', 'sly_dmyw','hxq_gjbs','hxq_gjtps','xmbhyc','yw_gkxfw','yw_nc','gbps','wcaqm','wcgz','xy','ywzt_yfyc','kgg_ybh' ]
    ...
    def inference(self, input_data, conf=0.5, end2end=False):
        # 判断 input_data 是图像路径还是图像帧
        if isinstance(input_data, str):  # 如果是字符串，假定是图像路径
            origin_img = cv2.imread(input_data)
        else:  # 否则，假定是直接的图像帧
            origin_img = input_data

        # 确保图像已成功加载或传入
        if origin_img is None:
            raise ValueError("Image not loaded properly or input frame is empty!")
