...
class C2fDWC(nn.Module):
    ...
    def forward(self, x):
        """Forward pass through C2f layer."""
        y = list(self.cv1(x).chunk(2, 1))  # 将输入分成两个部分
        y.extend(m(y[-1]) for m in self.m)  # 在最后一部分应用 Bottleneck
        return self.cv2(torch.cat(y, 1))  # 将结果连接并通过最后的卷积层

    def forward_split(self, x):
        """Forward pass using split() instead of chunk()."""
        y = list(self.cv1(x).split((self.c, self.c), 1))  # 使用 split() 方法分割
        y.extend(m(y[-1]) for m in self.m)  # 在最后一部分应用 Bottleneck
        return self.cv2(torch.cat(y, 1))  # 将结果连接并通过最后的卷积层
...
class BottleneckDWC(nn.Module):
    ...
    def __init__(self, c1, c2, shortcut=True, g=1, e=0.5):
        ...
        self.dw_conv1 = DWConv(c1, c_, 3, 1)  # 第一层深度可分离卷积

        # First 1x1 convolution
        self.conv1x1_1 = Conv(c_, c_, 1, 1)  # 第一层1x1卷积

        # Second depthwise separable convolution
        self.dw_conv2 = DWConv(c_, c_, 3, 1)  # 第二层深度可分离卷积

        # Second 1x1 convolution
        self.conv1x1_2 = Conv(c_, c2, 1, 1)  # 第二层1x1卷积
    ...
    def forward(self, x):
        ...
        dw_out2 = self.dw_conv2(conv_out1)  # 第二个深度可分离卷积
        final_output = self.conv1x1_2(dw_out2)  # 第二个1x1卷积
...
class BottleneckPC(nn.Module):
    ...
    #
    #     # First depthwise separable convolution
    #     self.p_conv1 = PConv(c1, c_)  # 第一层深度可分离卷积
    #
    #     # First 1x1 convolution
    #     self.conv1x1_1 = Conv(c_, c_, 1, 1)  # 第一层1x1卷积
    #
    #     # Second depthwise separable convolution
    #     self.p_conv2 = PConv(c_, c_)  # 第二层深度可分离卷积
    #
    #     # Second 1x1 convolution
    #     self.conv1x1_2 = Conv(c_, c2, 1, 1)  # 第二层1x1卷积
    #
    ...
    # def forward(self, x):
    #     """'forward()' applies the modified YOLO FPN to input data."""
    #     p_out1 = self.p_conv1(x)  # 第一个深度可分离卷积
    #     conv_out1 = self.conv1x1_1(p_out1)  # 第一个1x1卷积
    #     p_out2 = self.p_conv2(conv_out1)  # 第二个深度可分离卷积
    #     final_output = self.conv1x1_2(p_out2)  # 第二个1x1卷积
    #
    #     return x + final_output if self.add else final_output
...
class BottleneckCDW(nn.Module):
    ...
    def __init__(self, c1, c2, shortcut=True, g=1, e=0.5):
        ...
        # First 1x1 convolution
        self.conv1x1_1 = Conv(c1, c1, 1, 1)  # 第一层1x1卷积

        # First depthwise separable convolution
        self.dw_conv1 = DWConv(c1, c_, 3, 1)  # 第一层深度可分离卷积

        # Second 1x1 convolution
        self.conv1x1_2 = Conv(c_, c_, 1, 1)  # 第二层1x1卷积

        # Second depthwise separable convolution
        self.dw_conv2 = DWConv(c_, c2, 3, 1)  # 第二层深度可分离卷积

        self.add = shortcut and c1 == c2

    def forward(self, x):
        """'forward()' applies the modified YOLO FPN with swapped convolution positions to input data."""
        conv_out1 = self.conv1x1_1(x)  # 第一个1x1卷积
        dw_out1 = self.dw_conv1(conv_out1)  # 第一个深度可分离卷积
        conv_out2 = self.conv1x1_2(dw_out1)  # 第二个1x1卷积
        final_output = self.dw_conv2(conv_out2)  # 第二个深度可分离卷积
...
class BottleneckDWCA(nn.Module):
    ...
    def __init__(self, c1, c2, shortcut=True, g=1, e=0.5):
        ...
        self.add = shortcut and c1 == c2
        self.CA = CA(c2, c2, reduction=32)  # 加入CA模块
...
class CAConv(nn.Module):
    ...
    def forward(self, x):
        identity = x
        attention = self.ca(x)  # 计算CA注意力
        x = self.conv(x)  # 进行卷积操作
        out = identity * attention + x  # 合并原始输入和卷积输出
        return out
...
class Bottleneck_CAConv(nn.Module):
    ...
    def __init__(self, c1, c2, shortcut=True, g=1, k=(3, 3), e=0.5):
        ...
        c_ = int(c2 * e)  # hidden channels
        self.cv1 = Conv(c1, c_, k[0], 1)
        self.cv2 = CAConv(c_, c2)  # 使用CAConv
        self.add = shortcut and c1 == c2
...
class CA(nn.Module):
    def __init__(self, c1, c2, reduction):
        ...
        self.act = nn.Hardswish()  # 使用 Hardswish 激活函数
